...
bool H264Decoder::UpdateMaxNumReorderFrames(const H264SPS* sps) {
  if (sps->vui_parameters_present_flag && sps->bitstream_restriction_flag) {
    if (sps->max_num_reorder_frames > std::numeric_limits<size_t>::max())
      max_num_reorder_frames_ = std::numeric_limits<size_t>::max();
    else if (sps->max_num_reorder_frames < std::numeric_limits<size_t>::lowest())
      max_num_reorder_frames_ = std::numeric_limits<size_t>::lowest();
    else
      max_num_reorder_frames_ = sps->max_num_reorder_frames;
...
bool H264Decoder::ProcessSPS(int sps_id, bool* need_new_buffers) {
  ...
  *need_new_buffers = false;
  ...
  if (std::numeric_limits<int>::max() / width_mb < height_mb) {
    DVLOG(1) << "Picture size is too big: " << new_pic_size.width() << new_pic_size.height();
    return false;
  }

  if (new_pic_size.width() == pic_size_.width() && new_pic_size.height() == pic_size_.height()) {
  ...
  DVLOG(1) << "New picture size: " << pic_size_.width() << " " << pic_size_.height();
  ...
  while (1) {
    ...
    switch (curr_nalu_->nal_unit_type) {
      ...
      case H264NALU::kSPS: {
        ...
        par_res = parser_.ParseSPS(&sps_id, curr_nalu_.get());
        if (par_res != H264Parser::kOk)
          SET_ERROR_AND_RETURN();
      ...
      case H264NALU::kPPS: {
        ...
        par_res = parser_.ParsePPS(&pps_id, curr_nalu_.get());
        if (par_res != H264Parser::kOk)
          SET_ERROR_AND_RETURN();
