...
Size H264SPS::GetCodedSize() const {
...
H264Parser::Result H264Parser::ParseSPS(int* sps_id, H264NALU* nalu) {
  // See 7.4.2.1.
  int data;
  Result res;
  ...
  *sps_id = -1;
  ...
  if (sps->pic_order_cnt_type == 0) {
    ...
    long long offset_acc = 0;
    for (int i = 0; i < sps->num_ref_frames_in_pic_order_cnt_cycle; ++i) {
      READ_SE_OR_RETURN(&sps->offset_for_ref_frame[i]);
      offset_acc += sps->offset_for_ref_frame[i];
    }
    if ((offset_acc < std::numeric_limits<int>::lowest())
       || (offset_acc > std::numeric_limits<int>::max()))
      return kInvalidStream;
  ...
  sps->data = (uint8_t*)malloc(nalu->size);
  memcpy(sps->data, nalu->data, nalu->size);
  sps->size = nalu->size;
...
H264Parser::Result H264Parser::ParsePPS(int* pps_id, H264NALU* nalu) {
  // See 7.4.2.2.
  const H264SPS* sps;
  Result res;
  ...
  *pps_id = -1;
  ...
  pps->data = (uint8_t*)malloc(nalu->size);
  memcpy(pps->data, nalu->data, nalu->size);
  pps->size = nalu->size;
